...
//	result, _ := client.Search(context.Background(), "周杰伦", 10)
//	music, _ := client.ParseMusic(context.Background(), "123456", "lossless")
package netease
...
// keywords: 搜索关键词
// limit: 返回结果数量（1-100）
func (c *Client) Search(ctx context.Context, keywords string, limit int) (*models.SearchResponse, error) {
	return c.searchAPI.Search(ctx, keywords, limit)
}

// ParseMusic 解析单曲
// songID: 歌曲ID或URL
// level: 音质等级（standard/exhigh/lossless/hires/jyeffect/jymaster/sky/dolby）
func (c *Client) ParseMusic(ctx context.Context, songID string, level string) (*models.MusicResponse, error) {
	return c.musicAPI.Parse(ctx, songID, level)
}

// ParsePlaylist 解析歌单
// playlistID: 歌单ID或URL
func (c *Client) ParsePlaylist(ctx context.Context, playlistID string) (*models.PlaylistResponse, error) {
	return c.playlistAPI.Parse(ctx, playlistID)
}
